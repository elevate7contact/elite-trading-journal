...
function StableInput(props) {
  ...
  delete rest.value; delete rest.onChange; delete rest.style;
  var ref = useRef(null);
  var local = useState(value != null ? value : "");
  var lv = local[0]; var setLv = local[1];
  useEffect(function() {
    if (ref.current !== document.activeElement) setLv(value != null ? value : "");
  }, [value]);
  return React.createElement("input", Object.assign({}, rest, {
    ref: ref,
    value: lv,
    style: Object.assign({}, styInp, style || {}),
    onChange: function(e) { setLv(e.target.value); onChange(e.target.value); }
  }));
}
...
function StableTextarea(props) {
  ...
  return React.createElement("textarea", Object.assign({}, rest, {
    ref: ref,
    value: lv,
    style: Object.assign({}, styInp, {resize:"vertical"}, style || {}),
    onChange: function(e) { setLv(e.target.value); onChange(e.target.value); }
  }));
}
...
function Lbl(props) {
  return React.createElement("label", {
    style: {fontSize:11,color:TX3,display:"block",marginBottom:5,letterSpacing:"0.08em",textTransform:"uppercase"}
  }, props.c);
}

function Divider() {
  return React.createElement("div", {
    style: {height:1,background:"linear-gradient(to right,transparent,"+BD2+",transparent)",margin:"14px 0"}
  });
}

function SecLabel(props) {
  return React.createElement("div", {
    style: {fontSize:11,letterSpacing:"0.2em",color:G,marginBottom:16,textTransform:"uppercase"}
  }, props.c);
}

const FUTURES = {
  "MES (Micro E-mini S&P)":{dpp:5,tick:0.25,tv:1.25,note:"1/10 de ES"},
  "ES (E-mini S&P)":{dpp:50,tick:0.25,tv:12.5,note:"Full e-mini"},
  "MNQ (Micro E-mini Nasdaq)":{dpp:2,tick:0.25,tv:0.50,note:"1/10 de NQ"},
  "NQ (E-mini Nasdaq)":{dpp:20,tick:0.25,tv:5,note:"Full e-mini"},
  "MYM (Micro E-mini Dow)":{dpp:0.5,tick:1,tv:0.50,note:"1/10 de YM"},
  "YM (E-mini Dow)":{dpp:5,tick:1,tv:5,note:"Full e-mini"},
  "MGC (Micro Gold)":{dpp:10,tick:0.1,tv:1,note:"10 oz"},
  "GC (Gold Full)":{dpp:100,tick:0.1,tv:10,note:"100 oz"},
};

const FOREX = {
  "EUR/USD":{pv:10},"GBP/USD":{pv:10},"AUD/USD":{pv:10},
  "USD/CHF":{pv:10},"USD/CAD":{pv:7.7},"USD/JPY":{pv:9.1},
  "GBP/JPY":{pv:9.1},"EUR/JPY":{pv:9.1},"XAU/USD":{pv:1},
  "US30/CFD":{pv:1},"NAS100/CFD":{pv:1},"SP500/CFD":{pv:1},
};
...
function calcPos(opts) {
  var market=opts.market, asset=opts.asset, balance=opts.balance,
      riskPct=opts.riskPct, slMode=opts.slMode, slVal=opts.slVal, lotType=opts.lotType;
  var rUSD = (parseFloat(balance)||0) * (parseFloat(riskPct)||0) / 100;
  if (market === "Futuros") {
    var sp = FUTURES[asset]; if (!sp) return {};
    var pts = slMode==="puntos" ? parseFloat(slVal)||0 : slMode==="ticks" ? (parseFloat(slVal)||0)*sp.tick : slMode==="usd" ? (parseFloat(slVal)||0)/sp.dpp : 0;
    var rpc = pts * sp.dpp; if (!rpc) return {contracts:0,rpc:0,total:0,sp:sp,pts:0};
    var contracts = Math.max(1, Math.floor(rUSD/rpc));
    return {contracts:contracts, rpc:Math.round(rpc*100)/100, total:Math.round(contracts*rpc*100)/100, sp:sp, pts:Math.round(pts*100)/100};
  }
  if (market === "Forex/CFD") {
    var pair = FOREX[asset] || {pv:10};
    var pipValPerLot = pair.pv;
    var pips = slMode==="pips" ? parseFloat(slVal)||0 : slMode==="usd" ? (parseFloat(slVal)||0)/pipValPerLot : parseFloat(slVal)||0;
    var rawLots = pips>0 ? rUSD/(pips*pipValPerLot) : 0;
    var lots = Math.max(0.01, Math.round(rawLots*100)/100);
    var actualRisk = Math.round(lots*pips*pipValPerLot*100)/100;
    return {lots:lots, pvpl:Math.round(pipValPerLot*100)/100, total:actualRisk, pips:Math.round(pips*10)/10};
  }
  if (market === "Crypto") {
    var units = Math.round(rUSD/(parseFloat(slVal)||1)*10000)/10000;
    return {units:units, total:Math.round(rUSD*100)/100};
  }
  return {};
}
...
function scorePsych(a) {
  ...
  if(a.q1==="1")rv+=3; if(a.q2==="2")ov+=2; if(a.q2==="3")ov+=3;
  if(a.q3==="2")fe+=2; if(a.q3==="3")fe+=3; if(a.q4==="2")ad+=2; if(a.q4==="3")ad+=3;
  if(a.q5==="2")gr+=2; if(a.q5==="3")gr+=3; if(a.q6==="2")ad+=2; if(a.q6==="3")ad+=3;
  if(a.q7==="3")fe+=2; if(a.q8==="2")fo+=2; if(a.q8==="3")fo+=3;
  var p=[];
  if(rv>=2)p.push({name:"Revenge Trading",level:rv>=3?"Alto":"Medio",color:"#C84B4B",desc:"Sobreoperas tras perdidas para recuperar. Patron destructivo de cuenta."});
  ...
  if(fo>=2)p.push({name:"FOMO",level:fo>=3?"Alto":"Medio",color:"#C4862A",desc:"Miedo a perderte movimientos. Entradas tardias en malas posiciones."});
  if(gr>=2)p.push({name:"Avaricia",level:gr>=3?"Alto":"Medio",color:"#C84B4B",desc:"Maximizar ganancias mas alla del plan."});
  if(fe>=2)p.push({name:"Miedo / Manos de papel",level:fe>=3?"Alto":"Medio",color:"#C4862A",desc:"Gestionas mal posiciones por miedo. Mueves stops o cierras anticipado."});
  if(ad>=2)p.push({name:"Senales de adiccion",level:ad>=3?"Alto":"Medio",color:"#C84B4B",desc:"Patrones similares a ludopatia. Considera apoyo profesional."});
  if(p.length===0)p.push({name:"Perfil disciplinado",level:"Solido",color:G,desc:"Muestras buenos habitos. Mantente consistente y sigue creciendo."});
  return p;
}

function getPlan(problems, level, exp) {
  var names = problems.map(function(p){return p.name;}), t=[];
  if(level==="Novato"){t.push("Completa 30 dias en demo antes de operar capital real.");t.push("Estudia un solo setup y dominalo completamente.");t.push("Maximo 1-2 trades por dia. La paciencia es tu primer edge.");}
  if(level==="Avanzado"){t.push("Analiza tu data: en que sesion, par y dia rindes mejor?");t.push("Trabaja en escalar posiciones sin cambiar tu psicologia.");}
  ...
  if(names.some(function(n){return n.includes("Avaricia");}))t.push("TP fijo e inamovible. Configura el take profit antes de entrar.");
  ...
  if(names.some(function(n){return n.includes("adiccion");}))t.push("Establece horarios estrictos de trading. Cero operaciones fuera de ese horario.");
...
const NAV_ITEMS = [{id:"dashboard",label:"Panel"},{id:"pre_trade",label:"Pre-Trade"},{id:"during_trade",label:"Operar"},{id:"post_trade",label:"Post-Trade"},{id:"history",label:"Historial"},{id:"lot_calc",label:"Calculadora"},{id:"funding_rules",label:"Fondeo"},{id:"chat",label:"Mentor IA"},{id:"accounts",label:"Cuentas"}];

function makeAcc(id, name) {
  return {id:id,name:name,balance:"10000",type:"Personal",riskPct:"1",funding:{company:"",maxDailyDD:"",maxTotalDD:"",profitTarget:"",minDays:"",extraRules:""}};
}
...
export default function App() {
  ...
  var tradesS = useState([
    {id:1,date:"2024-03-10",pair:"EUR/USD",dir:"Long",entry:1.085,sl:1.082,tp:1.091,result:"Win",pnl:120,rr:2,emotion:"Calmado",followed:true,errors:["Ninguno"],notes:"Setup limpio",accountId:1},
    {id:2,date:"2024-03-11",pair:"US30",dir:"Short",entry:39200,sl:39350,tp:38900,result:"Loss",pnl:-75,rr:2,emotion:"Ansioso",followed:false,errors:["Movi el stop"],notes:"Mal momento",accountId:1},
    {id:3,date:"2024-03-12",pair:"XAUUSD",dir:"Long",entry:2180,sl:2170,tp:2205,result:"Win",pnl:250,rr:2.5,emotion:"Confiado",followed:true,errors:["Ninguno"],notes:"Tendencia clara",accountId:1},
    {id:4,date:"2024-03-14",pair:"MNQ",dir:"Short",entry:18200,sl:18280,tp:18000,result:"Win",pnl:400,rr:2.5,emotion:"Calmado",followed:true,errors:["Ninguno"],notes:"",accountId:1},
  ]);
  var trades=tradesS[0],setTrades=tradesS[1];
  ...
  var lcS = useState({market:"Forex/CFD",asset:"EUR/USD",balance:"10000",riskPct:"1",slMode:"pips",slVal:"20",lotType:"Micro"}); var lc=lcS[0],setLc=lcS[1];
  ...
  function answerQ(v) {
    var na = Object.assign({},answers,{[PSY[qIdx].id]:v});
    setAnswers(na);
    if(qIdx<PSY.length-1){setQIdx(qIdx+1);}
    else{var p=scorePsych(na);setPlan(getPlan(p,traderLevel,traderExp));setProfile(p);setPhase("profile_result");}
  }
  ...
  function getEquity() {
    var b=parseFloat(activeAcc.balance)||10000;
    return accTrades.map(function(t,i){b+=parseFloat(t.pnl)||0;return {name:"T"+(i+1),balance:Math.round(b)};});
  }

  function checkHealth() {
    var f=activeAcc.funding;
    if(!f.maxDailyDD||!f.maxTotalDD)return null;
  ...
  async function getAIFeedback(trade) {
    ...
    var p="Eres un coach de trading de elite. "+lvl+"\nAnaliza en espanol (max 180 palabras):\nPar: "+trade.pair+", Dir: "+trade.dir+", Resultado: "+trade.result+", PL: $"+trade.pnl+", RR: "+trade.rr+"\nEmocion: "+trade.emotion+", Siguio el plan: "+(trade.followed?"Si":"No")+", Errores: "+(trade.errors||[]).join(", ")+"\n1) Evaluacion 2) Patron psicologico 3) Tarea concreta. Tono mentor exigente y empatico.";
    try {
      var r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:p}]})});
      var d=await r.json();setAiFeedback(d.content&&d.content[0]?d.content[0].text:"");
  ...
  async function saveTrade() {
    ...
    var t=Object.assign({},curTrade,{id:Date.now(),date:new Date().toISOString().split("T")[0],rr:rr,accountId:activeAccId});
  ...
  async function sendChat() {
    ...
    try {
      var r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:sys,messages:newMsgs.map(function(m){return {role:m.role,content:m.content};})})});
      var d=await r.json();
      var reply=d.content&&d.content[0]?d.content[0].text:"Error.";
      setChatMsgs(newMsgs.concat([{role:"assistant",content:reply}]));
      var nm=Object.assign({},traderMem);
  ...
  var posResult=calcPos({market:lc.market,asset:lc.asset,balance:lc.balance,riskPct:lc.riskPct,slMode:lc.slMode,slVal:lc.slVal,lotType:lc.lotType});
    ...
    <div style={Object.assign({},wrap,{display:"flex",alignItems:"center",justifyContent:"center"})}>
      <div style={{textAlign:"center",padding:"2rem",maxWidth:440}}>
        ...
        <div style={{marginBottom:16}}>
          <Lbl c="Como quieres que te llame?" />
          <StableInput value={traderName} onChange={function(v){setTraderName(v);}} placeholder="Ej: Juan, El Condor, Trader07..." style={{textAlign:"center",fontSize:15}} />
        </div>
  ...
  if(phase==="level_select") return (
    <div style={Object.assign({},wrap,{padding:"2rem 1.5rem"})}>
      ...
      <div style={styCardG}>
        ...
        {["Novato","Intermedio","Avanzado"].map(function(l){
          return (
            <div key={l} onClick={function(){setTraderLevel(l);}} style={{padding:"14px 18px",border:"1px solid "+(traderLevel===l?G:BD),borderRadius:10,marginBottom:10,cursor:"pointer",background:traderLevel===l?"rgba(201,168,76,0.08)":"transparent"}}>
              <div style={{color:traderLevel===l?G:TX,fontSize:14}}>{l}</div>
              <div style={{fontSize:12,color:TX3,marginTop:3}}>{LEVEL_DESC[l]}</div>
            </div>
          );
        })}
      </div>
      <div style={styCardG}>
        ...
        {EXP_OPTS.map(function(e){
          return (
            <div key={e} onClick={function(){setTraderExp(e);}} style={{padding:"12px 18px",border:"1px solid "+(traderExp===e?G:BD),borderRadius:10,marginBottom:10,cursor:"pointer",background:traderExp===e?"rgba(201,168,76,0.08)":"transparent"}}>
              <div style={{color:traderExp===e?G:TX,fontSize:14}}>{e}</div>
            </div>
          );
        })}
      </div>
  ...
  if(phase==="quiz") {
    ...
    return (
      <div style={Object.assign({},wrap,{padding:"1.5rem"})}>
        ...
        <div style={{height:2,background:S3,borderRadius:2,marginBottom:20}}>
          <div style={{height:2,background:"linear-gradient(to right,"+G+","+G2+")",borderRadius:2,width:((qIdx+1)/PSY.length*100)+"%",transition:"width .4s"}} />
        </div>
        <div style={styCardG}>
          <div style={{fontSize:15,color:TX,marginBottom:20,lineHeight:1.7}}>{q.text}</div>
          {q.opts.map(function(opt,i){
            return (
              <div key={i} onClick={function(){answerQ(String(i));}} style={{padding:"12px 16px",borderRadius:9,border:"1px solid "+BD,marginBottom:9,cursor:"pointer",fontSize:13,color:TX2}}
                onMouseOver={function(e){e.currentTarget.style.borderColor=G;e.currentTarget.style.color=TX;e.currentTarget.style.background="rgba(201,168,76,0.06)";}}
                onMouseOut={function(e){e.currentTarget.style.borderColor=BD;e.currentTarget.style.color=TX2;e.currentTarget.style.background="transparent";}}>{opt}</div>
            );
          })}
        </div>
      </div>
  ...
  if(phase==="profile_result") return (
    <div style={Object.assign({},wrap,{padding:"1.5rem"})}>
      ...
      {profile.map(function(p,i){
        return (
          <div key={i} style={Object.assign({},styCard,{borderLeft:"2px solid "+p.color})}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
              <span style={{color:TX,fontSize:14}}>{p.name}</span>
              <span style={{padding:"3px 10px",borderRadius:20,background:p.color+"22",color:p.color,fontSize:11}}>{p.level}</span>
            </div>
            <div style={{fontSize:12,color:TX2,lineHeight:1.7}}>{p.desc}</div>
          </div>
        );
      })}
      <Divider />
        ...
        {plan.map(function(t,i){
          return (
            <div key={i} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:"1px solid "+BD}}>
              <span style={{color:G,fontSize:12,minWidth:18}}>{i+1}.</span>
              <span style={{fontSize:13,color:TX2,lineHeight:1.7}}>{t}</span>
            </div>
          );
        })}
      </div>
      <button style={styBtnP} onClick={function(){setPhase("account_setup");}}>Configurar mi cuenta</button>
    </div>
  );
  ...
  if(phase==="account_setup") return (
    <div style={Object.assign({},wrap,{padding:"1.5rem"})}>
      ...
      <div style={styCard}>
        ...
        <StableSelect value={activeAcc.type} onChange={function(v){updAcc("type",v);}} style={{marginBottom:12}}>
        ...
        <div style={{marginTop:12,padding:"10px 14px",background:"#000",borderRadius:8,fontSize:12,color:TX2,border:"1px solid "+BD}}>
          Riesgo por trade: <span style={{color:G}}>${Math.round((parseFloat(activeAcc.balance)||0)*(parseFloat(activeAcc.riskPct)||0)/100)}</span>
        </div>
      </div>
      {activeAcc.type==="Empresa de fondeo" && (
        <div style={styCard}>
          <div style={{color:G,fontSize:11,textTransform:"uppercase",marginBottom:14}}>Reglas de fondeo</div>
          <Lbl c="Empresa" /><StableInput placeholder="FTMO, The5ers..." value={activeAcc.funding.company} onChange={function(v){updFund("company",v);}} style={{marginBottom:12}} />
          ...
          <div style={{marginTop:12}}><Lbl c="Reglas adicionales" /><StableTextarea style={{height:70}} value={activeAcc.funding.extraRules} onChange={function(v){updFund("extraRules",v);}} placeholder="No operar NFP, max 5 trades/dia..." /></div>
        </div>
      )}
      <button style={styBtnP} onClick={function(){setPhase("dashboard");}}>Entrar al diario</button>
    </div>
  );
    ...
    <div style={wrap}>
      <div style={{background:"#111118",borderBottom:"1px solid "+BD}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px 6px"}}>
          <div>
            <span style={{fontSize:11,color:G,letterSpacing:"0.15em",textTransform:"uppercase"}}>Elite Trading Journal</span>
            <span style={{fontSize:13,color:TX,marginLeft:12}}>{traderName}</span>
            <span style={{fontSize:11,color:TX3,marginLeft:8}}>- {traderLevel}</span>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {accounts.map(function(a){
              return <button key={a.id} onClick={function(){setActiveAccId(a.id);}} style={{padding:"4px 10px",borderRadius:12,border:"1px solid "+(activeAccId===a.id?G:BD),background:activeAccId===a.id?"rgba(201,168,76,0.12)":"transparent",color:activeAccId===a.id?G:TX3,fontSize:10,cursor:"pointer",fontFamily:"Georgia,serif"}}>{a.name}</button>;
            })}
          </div>
        </div>
        <div style={{display:"flex",gap:2,flexWrap:"wrap",padding:"0 12px"}}>
          {NAV_ITEMS.map(function(n){
            return <button key={n.id} style={{padding:"6px 11px",borderRadius:"8px 8px 0 0",border:"1px solid "+(phase===n.id?BD2:"transparent"),borderBottom:"none",background:phase===n.id?S2:"transparent",color:phase===n.id?G:TX3,cursor:"pointer",fontSize:11,fontFamily:"Georgia,serif"}} onClick={function(){setPhase(n.id);}}>{n.label}</button>;
          })}
        </div>
      </div>
      ...
      <div style={{padding:"1.5rem"}}>
        ...
        {phase==="accounts" && (
          <div>
            ...
            {accounts.map(function(a){
              return (
                <div key={a.id} style={Object.assign({},styCard,{borderLeft:"2px solid "+(activeAccId===a.id?G:BD)})}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <span style={{color:activeAccId===a.id?G:TX,fontSize:14}}>{a.name}</span>
                    <div style={{display:"flex",gap:8}}>
                      {activeAccId!==a.id && <button style={Object.assign({},styBtn,{padding:"4px 10px",fontSize:11})} onClick={function(){setActiveAccId(a.id);}}>Activar</button>}
                      {accounts.length>1 && <button style={Object.assign({},styBtn,{padding:"4px 10px",fontSize:11,borderColor:"#C84B4B",color:"#C84B4B"})} onClick={function(){setAccounts(function(ac){return ac.filter(function(x){return x.id!==a.id;});});}}>Eliminar</button>}
                    </div>
                  </div>
                  <div style={{fontSize:12,color:TX2}}>Saldo: <span style={{color:G}}>${a.balance}</span> — Riesgo: {a.riskPct}% — {a.type}</div>
                </div>
              );
            })}
            <div style={styCardG}>
              ...
              <button style={Object.assign({},styBtnP,{marginTop:12})} onClick={function(){
                if(newAccName.trim()){var id=Date.now();setAccounts(function(ac){return ac.concat([makeAcc(id,newAccName.trim())]);});setNewAccName("");setActiveAccId(id);}
              }}>Crear cuenta</button>
            </div>
          </div>
        )}
        ...
        {phase==="dashboard" && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:10,marginBottom:14}}>
              {[{v:stats.wr+"%",l:"Win rate"},{v:"$"+stats.totalPnl,l:"P&L total",c:stats.totalPnl>=0?G:"#C84B4B"},{v:stats.total,l:"Trades"},{v:stats.avgRR+"R",l:"RR promedio"}].map(function(m,i){
                return <div key={i} style={{background:S3,border:"1px solid "+BD,borderRadius:10,padding:"1rem",textAlign:"center"}}><div style={{fontSize:22,color:m.c||G}}>{m.v}</div><div style={{fontSize:10,color:TX3,marginTop:5,letterSpacing:"0.08em",textTransform:"uppercase"}}>{m.l}</div></div>;
              })}
            </div>
            {stats.streak>1 && <div style={Object.assign({},styCard,{borderLeft:"2px solid "+(stats.sType==="Win"?G:"#C84B4B"),padding:"10px 16px",marginBottom:14})}>
              <span style={{fontSize:12,color:TX2}}>Racha actual: </span>
              <span style={{color:stats.sType==="Win"?G:"#C84B4B"}}>{stats.streak} {stats.sType==="Win"?"wins consecutivos":"losses consecutivos"}</span>
              {stats.sType==="Loss"&&stats.streak>=2 && <span style={{fontSize:11,color:"#C84B4B",marginLeft:12}}>- Considera pausar el dia</span>}
            </div>}
            {health && <div style={styCard}>
              <div style={{fontSize:11,color:G,marginBottom:12,textTransform:"uppercase",letterSpacing:"0.1em"}}>Salud fondeo — {activeAcc.funding.company}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                {[{l:"DD diario",u:health.du,lim:health.dl,p:health.dp},{l:"DD total",u:health.tl,lim:health.tlt,p:health.tp}].map(function(h){
                  return <div key={h.l}><div style={{fontSize:11,color:TX2,marginBottom:6}}>{h.l}: ${h.u} / ${h.lim}</div><div style={{height:6,background:S3,borderRadius:3}}><div style={{height:6,borderRadius:3,background:h.p>80?"#C84B4B":h.p>50?"#C4862A":G,width:Math.min(100,h.p)+"%",transition:"width .4s"}}/></div><div style={{fontSize:10,color:TX3,marginTop:3}}>{h.p}%{h.p>80?" — PELIGRO":""}</div></div>;
                })}
              </div>
            </div>}
            <div style={styCard}>
        ...
        {phase==="pre_trade" && (
          <div>
            ...
            <div style={styCard}>
              ...
              <Divider />
              <Lbl c="Sesion de mercado" />
              <StableSelect value={preTrade.session} onChange={function(v){setPreTrade(function(p){return Object.assign({},p,{session:v});});}} style={{marginBottom:12}}>
                <option value="">Seleccionar...</option>
                {SESSIONS.map(function(s){return <option key={s}>{s}</option>;})}
              </StableSelect>
              <Lbl c="Bias del dia" />
              <div style={{display:"flex",gap:8,marginBottom:12}}>
                {["Alcista","Bajista","Neutral"].map(function(b){
                  return <button key={b} onClick={function(){setPreTrade(function(p){return Object.assign({},p,{bias:b});});}} style={Object.assign({},styBtn,{flex:1,borderColor:preTrade.bias===b?G:BD,color:preTrade.bias===b?G:TX2,background:preTrade.bias===b?"rgba(201,168,76,0.08)":"transparent"})}>{b}</button>;
                })}
              </div>
        ...
        {phase==="during_trade" && (
          <div>
            ...
            {savedPre && <div style={Object.assign({},styCard,{borderLeft:"2px solid "+G,padding:"10px 16px",fontSize:12,marginBottom:14})}>
              <span style={{color:G}}>Plan activo</span><span style={{color:TX2,marginLeft:10}}>Bias: {preTrade.bias} — {preTrade.session} — Estado: {preTrade.mood}/10</span>
            </div>}
            <div style={styCard}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div><Lbl c="Activo / Par" />
                  <StableSelect value={curTrade.pair} onChange={function(v){setCurTrade(function(t){return Object.assign({},t,{pair:v});});}}>
                    <option value="">Seleccionar...</option>
                    {ALL_PAIRS.map(function(p){return <option key={p}>{p}</option>;})}
                  </StableSelect>
                </div>
                <div><Lbl c="Direccion" />
                  <div style={{display:"flex",gap:8}}>
                    {["Long","Short"].map(function(d){
                      var isActive=curTrade.dir===d;
                      var col=d==="Long"?"#6AAF5E":"#C84B4B";
                      return <button key={d} onClick={function(){setCurTrade(function(t){return Object.assign({},t,{dir:d});});}} style={Object.assign({},styBtn,{flex:1,borderColor:isActive?col:BD,color:isActive?col:TX2,background:isActive?(d==="Long"?"rgba(106,175,94,0.08)":"rgba(200,75,75,0.08)"):"transparent"})}>{d}</button>;
                    })}
                  </div>
                </div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:12}}>
                ...
                <div><Lbl c="RR estimado" />
                  <div style={{padding:"9px 12px",background:"#000",borderRadius:8,fontSize:15,color:G,border:"1px solid "+BD}}>
                    {curTrade.entry&&curTrade.sl&&curTrade.tp?Math.abs((parseFloat(curTrade.tp)-parseFloat(curTrade.entry))/(parseFloat(curTrade.entry)-parseFloat(curTrade.sl))).toFixed(2)+"R":"—"}
                  </div>
                </div>
                ...
                <div style={{marginTop:10}}><Lbl c="Resultado" />
                  <StableSelect value={curTrade.result} onChange={function(v){setCurTrade(function(t){return Object.assign({},t,{result:v});});}}>
                    <option value="">Seleccionar...</option><option>Win</option><option>Loss</option><option>BE</option>
                  </StableSelect>
                </div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
                {EMOTIONS.map(function(em){
                  return <button key={em} onClick={function(){setCurTrade(function(t){return Object.assign({},t,{emotion:em});});}} style={Object.assign({},styBtn,{padding:"5px 12px",fontSize:11,borderColor:curTrade.emotion===em?G:BD,color:curTrade.emotion===em?G:TX2,background:curTrade.emotion===em?"rgba(201,168,76,0.08)":"transparent"})}>{em}</button>;
                })}
              </div>
              <Lbl c="Seguiste el plan?" />
              <div style={{display:"flex",gap:8,marginBottom:14}}>
                {[true,false].map(function(v){
                  return <button key={String(v)} onClick={function(){setCurTrade(function(t){return Object.assign({},t,{followed:v});});}} style={Object.assign({},styBtn,{borderColor:curTrade.followed===v?G:BD,color:curTrade.followed===v?G:TX2,background:curTrade.followed===v?"rgba(201,168,76,0.08)":"transparent"})}>{v?"Si":"No"}</button>;
                })}
              </div>
              <Lbl c="Errores cometidos" />
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
                {ERRORS.map(function(err){
                  var isActive=(curTrade.errors||[]).includes(err);
                  return <button key={err} onClick={function(){setCurTrade(function(t){var e=t.errors||[];return Object.assign({},t,{errors:e.includes(err)?e.filter(function(x){return x!==err;}):[].concat(e,[err])});});}} style={Object.assign({},styBtn,{padding:"5px 12px",fontSize:11,borderColor:isActive?G:BD,color:isActive?G:TX2,background:isActive?"rgba(201,168,76,0.08)":"transparent"})}>{err}</button>;
                })}
              </div>
        ...
        {phase==="post_trade" && (
          <div>
            ...
            {aiFeedback && !aiLoading && <div style={Object.assign({},styCardG,{borderLeft:"2px solid "+G})}>
              <div style={{fontSize:11,color:G,marginBottom:12,textTransform:"uppercase",letterSpacing:"0.1em"}}>Analisis de tu mentor</div>
              <div style={{fontSize:13,color:TX2,lineHeight:1.9,whiteSpace:"pre-wrap"}}>{aiFeedback}</div>
            </div>}
        ...
        {phase==="history" && (
          <div>
            ...
            {accTrades.length===0 && <div style={{fontSize:13,color:TX3,textAlign:"center",padding:"2rem"}}>No hay operaciones en esta cuenta aun.</div>}
              ...
              return (
                <div key={t.id} style={Object.assign({},styCard,{borderLeft:"2px solid "+col})}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                    <div style={{fontSize:14,color:TX}}>{t.pair} <span style={{fontSize:12,color:TX3}}>{t.dir}</span></div>
                  ...
                  <div style={{fontSize:11,color:TX3}}>{t.date} — {t.rr}R — {t.emotion} — {t.followed?"Siguio el plan":"No siguio el plan"}{(t.errors||[]).filter(function(e){return e!=="Ninguno";}).length>0 && <span style={{color:"#C4862A"}}> — {(t.errors||[]).filter(function(e){return e!=="Ninguno";}).join(", ")}</span>}</div>
          ...
          <div>
            ...
            <div style={styCard}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                ...
                {lc.market==="Forex/CFD" && <div style={{padding:"8px 12px",background:"#000",borderRadius:8,border:"1px solid "+BD,fontSize:11,color:TX2,display:"flex",flexDirection:"column",justifyContent:"center"}}>
                  <div style={{color:G,marginBottom:2}}>Valor del pip (1 lot)</div>
                  <div>${(FOREX[lc.asset]||{pv:10}).pv} por pip por lote</div>
                </div>}
                {lc.market==="Futuros" && FUTURES[lc.asset] && <div style={{padding:"8px 12px",background:"#000",borderRadius:8,border:"1px solid "+BD,fontSize:11,color:TX2,display:"flex",flexDirection:"column",justifyContent:"center"}}>
                  <div style={{color:G,marginBottom:2}}>{FUTURES[lc.asset].note}</div>
                  <div>${FUTURES[lc.asset].dpp}/punto — tick ${FUTURES[lc.asset].tv}</div>
                </div>}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                <div><Lbl c="Medir SL en" />
                  <StableSelect value={lc.slMode} onChange={function(v){setLc(function(l){return Object.assign({},l,{slMode:v});}); }}>
              ...
              <div style={{padding:"18px",background:"#000",borderRadius:10,border:"1px solid "+BD2,textAlign:"center",marginBottom:14}}>
                {lc.market==="Futuros" && <div>
                  <div style={{fontSize:11,color:TX3,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Contratos recomendados</div>
                  <div style={{fontSize:36,color:G,fontFamily:"Georgia,serif"}}>{posResult.contracts||0}</div>
                  <div style={{fontSize:11,color:TX3,marginTop:4}}>Riesgo por contrato: ${posResult.rpc||0} — Stop: {posResult.pts||0} pts<br/>Riesgo total: <span style={{color:G}}>${posResult.total||0}</span></div>
                </div>}
                {lc.market==="Forex/CFD" && <div>
                  <div style={{fontSize:11,color:TX3,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Tamano de posicion</div>
                  <div style={{fontSize:36,color:G,fontFamily:"Georgia,serif"}}>{posResult.lots||0} <span style={{fontSize:16}}>lots</span></div>
                  <div style={{fontSize:11,color:TX3,marginTop:4}}>Valor del pip: ${posResult.pvpl||0}/pip — Stop: {posResult.pips||0} pips<br/>Riesgo real: <span style={{color:G}}>${posResult.total||0}</span></div>
                </div>}
                {lc.market==="Crypto" && <div>
                  <div style={{fontSize:11,color:TX3,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Unidades</div>
                  <div style={{fontSize:36,color:G}}>{posResult.units||0}</div>
                  <div style={{fontSize:11,color:TX3,marginTop:4}}>Riesgo total: <span style={{color:G}}>${posResult.total||0}</span></div>
                </div>}
              </div>
              ...
              {[0.5,1,1.5,2].map(function(r){
                var res=calcPos({market:lc.market,asset:lc.asset,balance:lc.balance,riskPct:String(r),slMode:lc.slMode,slVal:lc.slVal,lotType:lc.lotType});
                return (
                  <div key={r} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid "+BD,fontSize:12}}>
                    <span style={{color:TX3}}>{r}% — ${Math.round((parseFloat(lc.balance)||0)*r/100)}</span>
                    <span style={{color:TX}}>
                      {lc.market==="Futuros" && ((res.contracts||0)+" contratos")}
                      {lc.market==="Forex/CFD" && ((res.lots||0)+" lots")}
                      {lc.market==="Crypto" && ((res.units||0)+" unidades")}
                      <span style={{color:G,marginLeft:8}}>${Math.round((parseFloat(lc.balance)||0)*r/100)}</span>
                    </span>
                  </div>
                );
              })}
        ...
        {phase==="funding_rules" && (
          <div>
            ...
            {health && <div style={styCard}>
              <div style={{fontSize:11,color:G,marginBottom:12,textTransform:"uppercase",letterSpacing:"0.1em"}}>Estado actual</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                {[{l:"DD diario",u:health.du,lim:health.dl,p:health.dp},{l:"DD total",u:health.tl,lim:health.tlt,p:health.tp}].map(function(h){
                  return <div key={h.l}><div style={{fontSize:11,color:TX2,marginBottom:6}}>{h.l}: ${h.u} / ${h.lim}</div><div style={{height:6,background:S3,borderRadius:3}}><div style={{height:6,borderRadius:3,background:h.p>80?"#C84B4B":h.p>50?"#C4862A":G,width:Math.min(100,h.p)+"%",transition:"width .4s"}}/></div><div style={{fontSize:10,color:TX3,marginTop:3}}>{h.p}%{h.p>80?" — PELIGRO":""}</div></div>;
                })}
              </div>
              {activeAcc.funding.profitTarget && <div style={{marginTop:12,fontSize:12,color:TX2}}>Hacia el objetivo: <span style={{color:G}}>${Math.max(0,stats.totalPnl)} / ${activeAcc.funding.profitTarget} ({Math.round(Math.max(0,stats.totalPnl)/parseFloat(activeAcc.funding.profitTarget)*100)}%)</span></div>}
            </div>}
            <button style={styBtnP} onClick={function(){setPhase("dashboard");}}>Guardar</button>
          </div>
        )}
        ...
        {phase==="chat" && (
          <div>
                ...
                {chatMsgs.map(function(m,i){
                  return (
                    <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
                      <div style={{maxWidth:"80%",padding:"10px 14px",borderRadius:m.role==="user"?"12px 12px 2px 12px":"12px 12px 12px 2px",background:m.role==="user"?"rgba(201,168,76,0.12)":S3,border:"1px solid "+(m.role==="user"?BD2:BD),fontSize:13,color:m.role==="user"?TX:TX2,lineHeight:1.8,whiteSpace:"pre-wrap",fontFamily:m.role==="assistant"?"Georgia,serif":"system-ui,sans-serif"}}>
                        {m.content}
                      </div>
                    </div>
                  );
            ...
            <div style={{marginTop:10,display:"flex",flexWrap:"wrap",gap:6}}>
              {["Como mejorar mi disciplina?","Me siento frustrado hoy","Que errores debo corregir?","Necesito motivacion"].map(function(txt){
                return <button key={txt} onClick={function(){setChatInput(txt);}} style={{padding:"5px 12px",borderRadius:20,border:"1px solid "+BD,background:"transparent",color:TX3,fontSize:11,cursor:"pointer",fontFamily:"system-ui,sans-serif"}}>{txt}</button>;
              })}
            </div>
          </div>
        )}
