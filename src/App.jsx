...
function StableInput(props) {
  var value = props.value; var onChange = props.onChange; var style = props.style;
  var rest = Object.assign({}, props); delete rest.value; delete rest.onChange; delete rest.style;
  var ref = useRef(null);
  var local = useState(value != null ? value : ""); var lv = local[0]; var setLv = local[1];
...
function StableSelect(props) {
  var value = props.value; var onChange = props.onChange; var children = props.children; var style = props.style;
...
export default function App({ session }) {
  ...
  async function saveTrade(){
    ...
    setTraderMem(nm);
    if(userId){
      await apiCall({saveTrade:{trader_id:userId,account_id:activeAccId,pair:t.pair,direction:t.dir,entry:t.entry,sl:t.sl,tp:t.tp,result:t.result,pnl:t.pnl,rr:t.rr,emotion:t.emotion,followed:t.followed,errors:t.errors,notes:t.notes,trade_date:t.date}});
  ...
  if(phase==="level_select") return(
    <div style={Object.assign({},wrap,{padding:"2rem 1.5rem"})}>
      ...
      <div style={styCardG}>
        ...
        {["Novato","Intermedio","Avanzado"].map(function(l){return(
      ...
      <div style={styCardG}>
        ...
        {EXP_OPTS.map(function(e){return(
    ...
    return(
      <div style={Object.assign({},wrap,{padding:"1.5rem"})}>
        ...
        <div style={styCardG}>
          ...
          {q.opts.map(function(opt,i){return(
  ...
  if(phase==="profile_result") return(
    <div style={Object.assign({},wrap,{padding:"1.5rem"})}>
      ...
      {profile.map(function(p,i){return(
      ...
      <button style={styBtnP} onClick={async function(){
        setPhase("account_setup");
        await saveTraderProfile(traderLevel,traderExp,profile,plan,traderMem);
      }}>Configurar mi cuenta</button>
    </div>
  );
  ...
  if(phase==="account_setup") return(
    <div style={Object.assign({},wrap,{padding:"1.5rem"})}>
      ...
      <button style={styBtnP} onClick={async function(){
        if(userId){
          var r=await apiCall({saveAccount:{trader_id:userId,name:activeAcc.name,balance:activeAcc.balance,type:activeAcc.type,risk_pct:activeAcc.riskPct,funding:activeAcc.funding}});
          if(r&&r.id)setActiveAccId(r.id);
        }
        setPhase("dashboard");
      }}>Entrar al diario</button>
    </div>
  );
    ...
    <div style={wrap}>
      <div style={{background:"#111118",borderBottom:"1px solid "+BD}}>
        ...
        <div style={{display:"flex",gap:2,flexWrap:"wrap",padding:"0 12px"}}>
          {NAV_ITEMS.map(function(n){return <button key={n.id} style={{padding:"6px 11px",borderRadius:"8px 8px 0 0",border:"1px solid "+(phase===n.id?BD2:"transparent"),borderBottom:"none",background:phase===n.id?S2:"transparent",color:phase===n.id?G:TX3,cursor:"pointer",fontSize:11,fontFamily:"Georgia,serif"}} onClick={function(){setPhase(n.id);}}>{n.label}</button>;})}
        </div>
      </div>
      ...
      {phase==="accounts"&&(
        <div>
          ...
          <div style={styCardG}>
            ...
            <button style={Object.assign({},styBtnP,{marginTop:12})} onClick={async function(){
              if(newAccName.trim()){
                var newAcc={trader_id:userId,name:newAccName.trim(),balance:"10000",type:"Personal",risk_pct:"1",funding:{}};
                var r=await apiCall({saveAccount:newAcc});
                var id=r&&r.id?r.id:Date.now();
                setAccounts(function(ac){return ac.concat([makeAcc(id,newAccName.trim())]);});
                setNewAccName("");setActiveAccId(id);
              }
            }}>Crear cuenta</button>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={styCard}>
            ...
            <StableSelect value={preTrade.session} onChange={function(v){setPreTrade(function(p){return Object.assign({},p,{session:v});});}} style={{marginBottom:12}}>
              <option value="">Seleccionar...</option>{SESSIONS.map(function(s){return <option key={s}>{s}</option>;})}
            </StableSelect>
            <Lbl c="Bias del dia" />
            <div style={{display:"flex",gap:8,marginBottom:12}}>
              {["Alcista","Bajista","Neutral"].map(function(b){return <button key={b} onClick={function(){setPreTrade(function(p){return Object.assign({},p,{bias:b});});}} style={Object.assign({},styBtn,{flex:1,borderColor:preTrade.bias===b?G:BD,color:preTrade.bias===b?G:TX2,background:preTrade.bias===b?"rgba(201,168,76,0.08)":"transparent"})}>{b}</button>;})}
            </div>
      ...
      {phase==="history"&&(
        <div>
          ...
          {accTrades.slice().reverse().map(function(t){var col=t.result==="Win"?G:t.result==="Loss"?"#C84B4B":"#C4862A";return(
      ...
      {phase==="funding_rules"&&(
        <div>
          ...
          <button style={styBtnP} onClick={async function(){
            if(userId){await apiCall({saveAccount:{trader_id:userId,name:activeAcc.name,balance:activeAcc.balance,type:activeAcc.type,risk_pct:activeAcc.riskPct,funding:activeAcc.funding}});}
            setPhase("dashboard");
          }}>Guardar</button>
        </div>
      )}
            ...
            <div style={{height:400,overflowY:"auto",padding:"1.25rem",display:"flex",flexDirection:"column",gap:12}}>
              {chatMsgs.map(function(m,i){return(
                <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
                  <div style={{maxWidth:"80%",padding:"10px 14px",borderRadius:m.role==="user"?"12px 12px 2px 12px":"12px 12px 12px 2px",background:m.role==="user"?"rgba(201,168,76,0.12)":S3,border:"1px solid "+(m.role==="user"?BD2:BD),fontSize:13,color:m.role==="user"?TX:TX2,lineHeight:1.8,whiteSpace:"pre-wrap",fontFamily:m.role==="assistant"?"Georgia,serif":"system-ui,sans-serif"}}>{m.content}</div>
                </div>
              );})}
